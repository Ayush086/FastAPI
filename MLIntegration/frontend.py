import streamlit as st
import requests

API_URL = "http://localhost:8000/predict"

st.title('Insaurance Premium Category Prediction')

st.markdown("Enter your details here: ")

## fields
age = st.number_input("Age", min_value=1, max_value=119, value=30)
weight = st.number_input('Weight (kg)', min_value=1.0, value=65.0)
height = st.number_input('Height (m)', min_value=.5, max_value=2.5, value=1.6)
income_lpa = st.number_input('Annual Income (LPA)', min_value=0.1, value=8.0)
smoker = st.selectbox('Are you a smoker ?', options=[True, False])
city = st.text_input('City', value="Mumbai")
occupation = st.selectbox("Occupation", options=['retired', 'freelancer', 'student', 'government_job',
       'business_owner', 'unemployed', 'private_job'])

if st.button('Predict'):
    input_data = {
        'age': age,
        'weight': weight,
        'height': height,
        'income_lpa': income_lpa,
        'smoker': smoker,
        'city': city,
        'occupation': occupation
    }
    
    try: 
        response = requests.post(API_URL, json=input_data)
        if response.status_code == 200:
            result = response.json()
            st.success(f"Predicted Category: **{result['predicted_category']}**")
        else:
            st.error(f'API Error: {response.status_code} - {response.text}')
    except requests.exceptions.ConnectionError:
        st.error("Could not connect to server. Make sure it's running!")